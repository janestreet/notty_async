...
module Model : sig
  ...
  let constrain_cursor t =
    ...
    let cursor =
      Int.clamp_exn ~min:0 ~max:(dim_x - 1) c_x, Int.clamp_exn ~min:0 ~max:(dim_y - 1) c_y
    in
    if Pos.( <> ) cursor t.cursor then { t with cursor } else t
  ;;

  let set_dim t dim = constrain_cursor { t with dim }
  let set_cursor t cursor = constrain_cursor { t with cursor }
  let set_hover t pos = { t with hover = Some pos }

  let create ~dim ~cursor =
    { dim; cursor; hover = None; marks = Map.empty (module Pos); mark_color = A.green }
  ;;
  ...
  let choose_new_color t = { t with mark_color = rand_color () }
  ...
  let base_attr t pos =
    match t.hover with
    | None -> A.empty
    | Some hover -> if Pos.( <> ) pos hover then A.empty else A.bg A.blue
  ;;
  ...
  let render t =
    ...
    let board =
      ...
      match t.hover with
      | None -> board
      | Some hover -> board |> place (I.string (base_attr t hover) ".") hover
    in
...
let run () =
  ...
  don't_wait_for
    (Pipe.iter_without_pushback events ~f:(fun ev ->
       match ev with
       | `Mouse (`Release, pos, _) -> m := Model.set_cursor !m pos
       | `Mouse (`Drag, pos, _) ->
       ...
       | `Key key ->
         (match key with
          ...
          | `ASCII 'c', [] -> m := Model.choose_new_color !m
          | _ -> ())
